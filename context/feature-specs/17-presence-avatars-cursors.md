...
2. Add the participant avatar group inside the editor canvas area.
    ...
    - render the current user separately using the existing Clerk UserButton 
    - do not render a second avatar for them from
    ...
    - collabortor avatars are display-only, not interactive
