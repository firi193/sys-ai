Add a small floating color toolbar so selected nodes can change both their background and text color directly on the
canvas.
...
Reuse existing theme colors if they already exist in the `global.css`. Otherwise, keep the palette in the canvas types/
constants, such as `types/canvas.ts`.
