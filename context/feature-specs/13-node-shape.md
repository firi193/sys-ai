Replace the placeholder node randerer with proper shape rendering and a drap preview.

## Implementation

1. Peplace the placeholder node shape rendering.
...
2. Add a shape drag preview.
    - when drapping a shape from the shape panel, show a ghost preview of that shape
    - keep the preview attached to the cursor while dragging.
    - use the same shape type and default size that will be used on drop
    - hide the rpeview after the shape is dropped or the drag is cancelled.
...
- CSS chapes render correctly for rectangle, pill, and circle.
- SVG shapes render and scale correctly for diamond, hexagon, and cylinder.
- Shape dragging shows a ghost preview matching the drapping shape.
